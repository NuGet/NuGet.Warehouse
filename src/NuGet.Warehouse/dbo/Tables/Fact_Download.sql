CREATE TABLE [dbo].[Fact_Download] (
    ...
    CONSTRAINT [PK_Fact_Download] PRIMARY KEY CLUSTERED ([Dimension_UserAgent_Id] ASC, [Dimension_Package_Id] ASC, [Dimension_Date_Id] ASC, [Dimension_Time_Id] ASC, [Dimension_Operation_Id] ASC, [Dimension_Project_Id] ASC) WITH (STATISTICS_NORECOMPUTE = ON)
);
...
CREATE NONCLUSTERED INDEX [Fact_Download_NCI_Package_Id]
    ...
    INCLUDE([Dimension_UserAgent_Id], [Dimension_Date_Id], [Dimension_Operation_Id], [DownloadCount]) WITH (STATISTICS_NORECOMPUTE = ON);
