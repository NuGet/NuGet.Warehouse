CREATE TABLE [dbo].[Dimension_UserAgent] (
    ...
    CONSTRAINT [PK_Dimension_UserAgent] PRIMARY KEY CLUSTERED ([Id] ASC) WITH (STATISTICS_NORECOMPUTE = ON)
);


GO
CREATE UNIQUE NONCLUSTERED INDEX [Dimension_UserAgent_2]
    ON [dbo].[Dimension_UserAgent]([Value] ASC) WITH (STATISTICS_NORECOMPUTE = ON);


GO
CREATE NONCLUSTERED INDEX [Dimension_UserAgent_3]
    ON [dbo].[Dimension_UserAgent]([Client] ASC) WITH (STATISTICS_NORECOMPUTE = ON);


GO
CREATE NONCLUSTERED INDEX [Dimension_UserAgent_4]
    ON [dbo].[Dimension_UserAgent]([ClientMajorVersion] ASC, [ClientMinorVersion] ASC) WITH (STATISTICS_NORECOMPUTE = ON);


GO
CREATE NONCLUSTERED INDEX [Dimension_UserAgent_5]
    ON [dbo].[Dimension_UserAgent]([ClientCategory] ASC) WITH (STATISTICS_NORECOMPUTE = ON);
